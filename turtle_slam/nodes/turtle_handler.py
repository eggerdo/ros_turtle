...
class TurtleHandler (object):
    ...
    def __init__(self):
        ...
        self.roomba = Roomba(self.address)
...
#            print "x=%.3f, y=%.3f, th=%.3f" %(self.x, self.y, self.th)
    ...
    def handleVelocity(self):
        ...
        if self.req_cmd_vel is not None:
            ...
            if self.roomba.mode != 1:
                self.roomba.setSafeMode()
...
#        self.roomba.drive(*req_cmd_vel)
        self.drive(*req_cmd_vel)
        self.last_cmd_vel = req_cmd_vel
        
    def drive(self, velocity, angle):
#        rospy.loginfo("drive v=%d, a=%d" %(velocity, angle))
        if velocity == 0:
            if angle > 0:
            ...
            self.roomba.drive(velocity, angle)
